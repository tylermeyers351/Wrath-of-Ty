...
Shader "Polytope Studio/PT_Vegetation_Opaque_Shader_NR"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				float4 appendResult632 = (float4(( ( ( ( simplePerlin2D622 - 0.5 ) / 10.0 ) * _WindStrength ) + v.vertex.x ) , v.vertex.y , v.vertex.z , 1.0));
				#ifdef _WINDMASKONOFF_ON
				float staticSwitch633 = ( ( 1.0 - v.texcoord.y ) * 5.0 );
				#else
				float staticSwitch633 = v.vertex.y;
				#endif
				float4 lerpResult635 = lerp( v.vertex , appendResult632 , staticSwitch633);
				...
				#else
				float4 staticSwitch638 = v.vertex;
				#endif
						 ...
						 ) : SV_Target
			{
				...
				float clampResult555 = clamp( pow( ( (0.5 + (IN.ase_texcoord8.y - 0.0) * (2.0 - 0.5) / (1.0 - 0.0)) * _Gradient ) , _GradientPower ) , 0.0 , 1.0 );
...
ASEEND*/
//CHKSM=393CB417DFFAA3480A14BE6CE67644ED26A0530C